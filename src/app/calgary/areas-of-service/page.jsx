...
// Calgary area cities for routing
const calgaryAreas = ['Calgary'];
...
// Function to sort locations with Calgary areas first
const sortLocationsByRegion = (locations: string[], prioritizeCalgary = true) => {
  if (!prioritizeCalgary) return locations;

  const calgaryLocs = locations.filter(loc => calgaryAreas.includes(loc));
  const otherLocs = locations.filter(loc => !calgaryAreas.includes(loc));

  return [...calgaryLocs, ...otherLocs];
};
...
const ServiceAreasPage = () => {
  ...
  // Get sorted locations - you can change this to true for Calgary pages
  const getSortedLocations = (serviceType: ServiceType) => {
    const locations = serviceAreas[serviceType] || [];
    return sortLocationsByRegion(locations, true); // Set to true for Calgary pages
  };
