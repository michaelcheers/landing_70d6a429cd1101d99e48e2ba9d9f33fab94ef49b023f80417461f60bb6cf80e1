...
import HamiltonTracker from '/src/app/components/HamiltonTracker.jsx';
import "../../landing.css";
import "../../landingcomm.css";

const Footer = dynamic(() => import("/src/app/components/footer.jsx"));

// Define custom content for specific service-area combinations
const CUSTOM_WHY_CHOOSE_CONTENT: { [key: string]: string } = {
...
// Function to parse slug and extract service type and city
function parseSlug(slug: string): { serviceType: string; citySlug: string } | null {
  // Try to match known service types from the beginning of the slug
  const serviceTypes = Object.keys(SERVICE_CONFIGS);

  for (const serviceType of serviceTypes) {
    // Check if slug starts with this service type followed by a hyphen
    if (slug.startsWith(serviceType + '-')) {
      const citySlug = slug.substring(serviceType.length + 1);

      // Make sure we have a city slug after the service type
...
// Helper function to get city name from various slug formats
function getCityNameFromSlug(citySlug: string): string {
  // Convert slug back to proper city name format
  return citySlug
    .split('-')
...
// Helper function to find service area by city name
function findServiceAreaByCityName(cityName: string) {
  // First try direct slug match
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const directMatch = Object.entries(SERVICE_AREAS).find(([slug, area]) => 
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const nameMatch = Object.entries(SERVICE_AREAS).find(([slug, area]) => 
    area.city.toLowerCase() === cityName.toLowerCase()
  );
...
: { 
  ...
  const { city: cityName2, province, region } = serviceArea;
  const { title: serviceTitle } = serviceConfig;
  
  return {
    title: `${serviceTitle} in ${cityName2}, ${province} | Moving Papa`,
    description: `Professional ${serviceTitle.toLowerCase()} services in ${cityName2}, ${province}. Expert movers serving ${region}. Licensed, insured, and trusted by hundreds. Get your free quote today!`,
    keywords: `${serviceTitle.toLowerCase()} ${cityName2.toLowerCase()}, moving company ${cityName2.toLowerCase()}, ${cityName2.toLowerCase()} movers, ${serviceTitle.toLowerCase()} services ${province.toLowerCase()}, ${region.toLowerCase()} movers`,
    openGraph: {
      title: `${serviceTitle} in ${cityName2}, ${province} | Moving Papa`,
      description: `Professional ${serviceTitle.toLowerCase()} services in ${cityName2}, ${province}. Expert movers serving ${region}.`,
      type: 'website',
    },
  };
}
...
export default async function ServiceAreaServicePage({ 
  ...
  const getQuoteComponent = () => {
    ...
    const commFromProp = isVancouverArea
      ? (serviceType === 'last-mile' ? 'vancouver-lastmile' : 'vancouver-moving')
      : (serviceType === 'last-mile' ? 'lastmile' : 'moving');

    const getQuoteFromProp = isVancouverArea
      ? (serviceType === 'storage' ? 'vancouver-storage' : serviceType === 'last-mile' ? 'lastmile' : 'vancouver-moving')
      : (serviceType === 'storage' ? 'storage' : serviceType === 'last-mile' ? 'lastmile' : 'moving');

    // Use CommGetQuote for last-mile, GetQuote for others
  ...
  return (
    ...
    <div className="fixed top-0 left-0 w-full z-10000">
        ...
        <div className="w-full flex justify-center pt-[60px]">
                ...
                <div className="w-full pb-5 grid grid-cols-1 md:grid-cols-2 md:pl-5 md:pt-10">
                    ...
                    <div className="cols-span-1 m-2 md:m-0 pr-2 md:ml-20 mt-5 md:mt-10">   
                        ...
                        {title} in <span className="text-tertiary">{cityName2}, {province}</span>
                        </div>
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-primary mb-4">
              Why Choose Moving Papa for {title} in {cityName2}?
            </h2>
            <div className="text-lg text-gray-600 max-w-5xl mx-auto text-left">
              {(() => {
                ...
                return <p>Our team brings years of experience and local expertise to every move in {region}. Here&apos;s what sets us apart:</p>;
              })()}
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-gray-600 max-w-3xl mx-auto">
              Based in {cityName2}, we provide {title.toLowerCase()} services throughout {region} and beyond. 
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
              ...
              <p className="text-gray-600">Deep knowledge of {cityName2} and {region} areas and regulations</p>
            </div>
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="bg-primary rounded-lg p-8 text-center">
            ...
            <p className="text-xl text-white mb-6">
              Contact us today for your free {title.toLowerCase()} quote in {cityName2}, {province}
            </p>
      ...
      {/* Get Quote Footer */}
      <div className="w-full bg-[#F8F5EC]" >
