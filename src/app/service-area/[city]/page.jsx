// src/app/service-area/[city]/page.tsx
...
import VideoReview from '/src/app/components/videoReview.jsx';
...
const Review = dynamic(() => import("/src/app/components/review.jsx"))
...
: { 
  ...
  if (!serviceArea) {
    return {
      title: 'Service Area Not Found - Moving Papa',
  ...
  return {
    title: `Professional Movers ${cityName}, ${province} | Moving Papa`,
    description: `Expert moving services in ${cityName}, ${province}. Professional movers serving ${region}. Licensed, insured, and trusted by hundreds. Get your free quote today!`,
    keywords: `movers ${cityName.toLowerCase()}, moving company ${cityName.toLowerCase()}, ${cityName.toLowerCase()} movers, moving services ${province.toLowerCase()}, ${region.toLowerCase()} movers`,
    openGraph: {
      title: `Professional Movers ${cityName}, ${province} | Moving Papa`,
      description: `Expert moving services in ${cityName}, ${province}. Professional movers serving ${region}.`,
      type: 'website',
      locale: 'en_CA',
    },
    alternates: {
...
export default async function ServiceAreaPage({ params }: ServiceAreaPageProps) {
  ...
  // Custom headings based on the city
  ...
  return (
    <div className="bg-[#F8F5EC]">
      <HamiltonTracker slug={city} />  
      <div className="fixed top-0 left-0 w-full z-10000">
        {isVancouver ? (<VanHeader />): (<Header />)}
      </div>
        ...
        <div className="w-full max-w-[1400px] rounded-4xl flex flex-col justify-center items-center py-10 md:pt-5 md:pb-20 mx-2">
          <div className="font-bold flex flex-col items-center text-primary">
            ...
            <div className="hidden md:flex flex-col">
              <h1 className="text-3xl md:text-4xl text-center">
                {customHeadings.subHeading}
              </h1>

              <div className="text-base md:text-lg font-regular mt-4 text-center">
                {customHeadings.description}<br />
                <p className="flex justify-center">Get a free quote within minutes</p>            
              </div>            
            </div>
          </div>
          <div className="w-10/11 max-w-[1200px] md:flex md:mt-5 justify-between">
            <div className="hidden md:flex items-center w-18/40">
              <div className="flex">
                <Image src={Heart} alt="Moving services" />
              </div>     
            </div>
            <div className="w-full md:w-20/40">
              <GetQuote from={isVancouver ? "vancouver-moving" : "moving"} />
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="pb-10">
            <div className="ml-2 pb-2 font-bold text-[16px] md:text-[22px] border-b-1 md:flex text-center">
              <div className="text-xl md:text-3xl font-bold text-primary">
                Hear from our {cityName} Customers, &nbsp;
              </div>
              <div className="text-xl md:text-3xl font-bold text-tertiary">
                the heart of our Success.
              </div>
            </div>
            <div className={`grid grid-cols-2 ${city === 'hamilton' ? 'md:grid-cols-5' : 'md:grid-cols-4'} mt-4 md:mt-8 gap-1 md:gap-x-4 font-bold md:w-8/10 mx-2`}>
              ...
              {city === 'hamilton' && (
                <div className="bg-white border-2 pl-2 flex items-center h-[60px] rounded-lg">
                  <div className="flex justify-center items-center bg-primary w-[40px] h-[40px] relative rounded-lg">
                    <Image src={GreenStorageLogo} alt="" fill sizes="100%" style={{ objectFit: "contain" }}/>
                  </div>
                  <div className="ml-3 md:ml-2 text-[14px] font-regular md:text-[18px]">
                    Green Storage
                  </div>
      ...
      <div className="flex flex-col items-center pt-5 md:pb-10 bg-primary">
        ...
        (
          ...
          <VideoReview from="/finalstep/residential"/>
        )
        }
      </div>
      ...
      <div className="my-5 mb-15">
        ...
        <div className="flex justify-center">
          <Link href={finalstepUrl} className="bg-tertiary font-bold text-white w-8/10 md:w-1/3 py-3 text-center rainbow-button !rounded-lg">Get a Quote</Link>
        </div>
      </div>      

      <div className="my-10 mt-8">
        <MovingServicesGrid from={fromProp} />           
      </div>
        ...
        >
          <div className="flex items-start text-white pt-10 pl-5 md:pl-10 md:w-[1254px]">
            <div className="flex flex-col items-start justify-center">
              ...
              <h1 className="italic text-2xl md:text-4xl">Behind in {cityName}.</h1>
              <span className="text-base font-regular mt-4 text-start md:w-[490px] pr-6 md:pr-0">
                At Moving Papa, we believe every move in {cityName} should leave the smallest footprint possible. We use eco-friendly materials, efficient routes, and sustainable practices to reduce waste and emissions — without compromising service.
              </span>
            </div>
          </div>
        </div>
