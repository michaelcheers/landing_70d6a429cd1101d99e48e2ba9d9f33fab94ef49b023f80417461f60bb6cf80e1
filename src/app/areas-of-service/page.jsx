...
// Vancouver area cities for routing
const vancouverAreas = ['Vancouver', 'Burnaby', 'Richmond', 'Surrey', 'Kelowna', 'Victoria', 'Vancouver Island'];
...
// Function to sort locations with Vancouver areas first
const sortLocationsByRegion = (locations: string[], prioritizeVancouver = false) => {
  if (!prioritizeVancouver) return locations;
  
  const vancouverLocs = locations.filter(loc => vancouverAreas.includes(loc));
  const otherLocs = locations.filter(loc => !vancouverAreas.includes(loc));
  
  return [...vancouverLocs, ...otherLocs];
};
...
const ServiceAreasPage = () => {
  ...
  // Get sorted locations - you can change this to true for Vancouver pages
  const getSortedLocations = (serviceType: ServiceType) => {
    const locations = serviceAreas[serviceType] || [];
    return sortLocationsByRegion(locations, false); // Set to true for Vancouver pages
  };
