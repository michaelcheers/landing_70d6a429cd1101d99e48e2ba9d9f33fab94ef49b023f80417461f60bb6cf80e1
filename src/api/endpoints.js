const SERVICE1_BASE = "https://helloservice1.com";
const SERVICE1_COMPANY_ID = 23;
const MP_LOCATION_TORONTO = 4;
const MP_LOCATION_VANCOUVER = 25;
const MP_LOCATION_OTTAWA = 36;
const MP_LOCATION_CALGARY = 37;
const MP_LOCATION_EDMONTON = 38;
const VANCOUVER_BRANCH_ID = "98a3854c-cb46-4025-b0a1-b265013b069e";
const VALID_LOCATIONS = /* @__PURE__ */ new Set([
  MP_LOCATION_TORONTO,
  MP_LOCATION_VANCOUVER,
  MP_LOCATION_OTTAWA,
  MP_LOCATION_CALGARY,
  MP_LOCATION_EDMONTON
]);
function resolveCompanyLocationId(body) {
  ...
  switch (String(body.City || "").toLowerCase()) {
    case "vancouver":
      return MP_LOCATION_VANCOUVER;
    case "ottawa":
      return MP_LOCATION_OTTAWA;
    case "calgary":
      return MP_LOCATION_CALGARY;
    case "edmonton":
      return MP_LOCATION_EDMONTON;
    case "toronto":
      return MP_LOCATION_TORONTO;
  }
  return body.BranchId === VANCOUVER_BRANCH_ID ? MP_LOCATION_VANCOUVER : MP_LOCATION_TORONTO;
}
function resolveCustomerTimezone(body) {
  switch (String(body.City || "").toLowerCase()) {
    case "vancouver":
...
function formatMoveDate(yyyymmdd) {
  if (yyyymmdd && yyyymmdd.length === 8) {
    return yyyymmdd.slice(0, 4) + "-" + yyyymmdd.slice(4, 6) + "-" + yyyymmdd.slice(6, 8);
  }
  return "";
}
function inferCompanyLocationFromUrl() {
...
function buildService1Payload(body) {
  ...
  if (body.ServiceType === "Commercial") {
    businessType = "Commercial Moving";
...
async function postOnlineBooking(body) {
  const payload = {
    ...
    companyLocationId: VALID_LOCATIONS.has(Number(body.companyLocationId)) ? Number(body.companyLocationId) : inferCompanyLocationFromUrl()
  };
...
async function getOnlineBookingConfig(companyId) {
...
export {
  ...
  formatMoveDate,
  getOnlineBookingConfig,
  inferCompanyLocationFromUrl,
  postBooking,
  postOnlineBooking,
  resolveCompanyLocationId,
